    ...
    <div class="row">
      <form class="col s12" action="{{ route('aluno.update', $aluno->id) }}" method="POST">
          ...
          <div class="input-field col s12 m6 l3">
            <select id="turma_id" name="turma_id">
              ...
              @foreach ($turmas as $turma)
                <option
                  value="{{ $turma->id }}" {{ $turma->id == $aluno->turma_id ? 'selected' : '' }}>{{ $turma->nome }}</option>
              @endforeach
            </select>
            <label>Turma</label>
          </div>
        ...
        <div class="row">
          ...
          <div class="input-field col s12 m3">
            <input id="password-confirm" type="password" name="password_confirmation" placeholder="Confirmar Nova Senha"
                   required>
            ...
            <p>
              <label>
                <input id='masculino' name="sexo" type="radio"
                       value="Masculino" {{ $aluno->sexo == "Masculino" ? 'checked' : '' }}/>
                <span>Masculino</span>
              </label>
              <label>
                <input id='feminino' name="sexo" type="radio"
                       value="Feminino" {{ $aluno->sexo == "Feminino" ? 'checked' : '' }}/>
                <span>Feminino</span>
              </label>
            </p>
          </div>
